...
#    addMyStream()
    ...
    for stream in  sorted(live_streams, key=lambda tup: tup[0]):        
        addStream(stream[0],'/live_now',101,stream[1],stream[5],stream[2],stream[3],stream[4], video_id='LIVE') 
...
def search(search_txt=''):    
    ...
    if search_txt != '':
        ...
        json_search = '{"requests":[{"indexName":"events","params":"query='+search_txt+'&hitsPerPage='+SEARCH_HITS+'"}],"apiKey":"'+API_KEY+'","appID":"'+APP_ID+'"}'        
    ...
    if json_source != '':
        ...
        for hits in json_source['results']: 
            ...
            for event in hits['hits']:                
                try:             
                    if 'is_live' in event and event['is_live'] == 1:
                        owner_id = str(event['owner_account_id'])                        
                        ...
                        name = '[COLOR=FF00B7EB]'+name+'[/COLOR]'
                        addStream(name,'/live_now',101,icon,FANART,event_id,owner_id,info, video_id='LIVE')
                except:
                    pass
